...
from core.model.net_mfb import Net_mfb
...
class Execution:
    ...
    def train(self, dataset, dataset_eval=None):
        ...
        gen_funcs = {"softmax": torch.softmax,"sparsemax": partial(sparsemax, k=512),"tvmax": "tvmax"}

        gen_func = gen_funcs[self.__C.gen_func]

        # Define the MCAN model
        if self.__C.MODEL=='mca':
            net = Net(self.__C,pretrained_emb,token_size,ans_size,gen_func=gen_func)
        else:
            net = Net_mfb(self.__C,pretrained_emb,token_size,ans_size,gen_func=gen_func)
        ...
        if self.__C.SHUFFLE_MODE in ['external']:
            dataloader = Data.DataLoader(dataset,batch_size=self.__C.BATCH_SIZE,shuffle=False,num_workers=self.__C.NUM_WORKERS,pin_memory=self.__C.PIN_MEM,drop_last=True
            )
        else:
            dataloader = Data.DataLoader(dataset,batch_size=self.__C.BATCH_SIZE,shuffle=True,num_workers=self.__C.NUM_WORKERS,pin_memory=self.__C.PIN_MEM,drop_last=True
            )
        ...
        for epoch in range(start_epoch, self.__C.MAX_EPOCH):
            ...
            for step, (img_feat_iter,ques_ix_iter,ans_iter,size) in enumerate(dataloader):
                ...
                for accu_step in range(self.__C.GRAD_ACCU_STEPS):
                    ...
                    sub_img_feat_iter = img_feat_iter[accu_step * self.__C.SUB_BATCH_SIZE:(accu_step + 1) * self.__C.SUB_BATCH_SIZE]
                    sub_ques_ix_iter = ques_ix_iter[accu_step * self.__C.SUB_BATCH_SIZE:(accu_step + 1) * self.__C.SUB_BATCH_SIZE]
                    ...
                    if self.__C.VERBOSE:
                        ...
                        print("\r[version %s][epoch %2d][step %4d/%4d][%s] loss: %.4f, lr: %.2e" % (
                            self.__C.VERSION,epoch + 1,step,int(data_size / self.__C.BATCH_SIZE),mode_str,
                            loss.cpu().data.numpy() / self.__C.SUB_BATCH_SIZE,optim._rate), end='          ')
            ...
            torch.save(state,self.__C.CKPTS_PATH +'ckpt_' + self.__C.VERSION +'/epoch' + str(epoch_finish) +'.pkl')
            ...
            if dataset_eval is not None:
                acc = self.eval(dataset_eval,state_dict=net.state_dict(),valid=True)
                if acc>=best_acc:
                    best_acc=acc
                    patience=0
                else:
